...
import { BookOpen, FileText, Video, Book, Bell, MessageSquare, Heart, Download, Bookmark, Calendar } from 'lucide-react';
    ...
    >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="glass py-2 px-4 rounded-lg text-sm font-medium">
              <div className="flex items-center">
                ...
                <span>
                  {user?.branch || 'Computer Science'} | Year {user?.year || '3'} | Semester {user?.semester || '5'}
                </span>
              </div>
            </div>
          </div>
      ...
      <motion.section variants={itemVariants} className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link to="/student/notes" className="glass-card p-4 text-center hover:shadow-lg transition-shadow">
          ...
          <Link to="/student/videos" className="glass-card p-4 text-center hover:shadow-lg transition-shadow">
          ...
          <Link to="/student/syllabus" className="glass-card p-4 text-center hover:shadow-lg transition-shadow">
          ...
          <Link to="/student/pyqs" className="glass-card p-4 text-center hover:shadow-lg transition-shadow">
        ...
        <motion.section variants={itemVariants} className="lg:col-span-2">
          <div className="glass-card p-6 rounded-xl h-full">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link to="/student/notes" className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                View All
              </Link>
            </div>
                    ...
                    {recentResources.notes.map(note => (
                      ...
                      >
                        ...
                        <div className="flex space-x-3 text-slate-500 dark:text-slate-400">
                          ...
                          <span className="flex items-center text-xs">
                            <Download size={12} className="mr-1" /> {note.downloads}
                          </span>
                        </div>
                      </Link>
                    ))}
                  ...
                  <div className="space-y-2">
                    {recentResources.pyqs.map(pyq => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-medium text-sm line-clamp-1">{pyq.title}</h4>
                          <p className="text-xs text-slate-500 dark:text-slate-400">
                            {pyq.subject} • {pyq.examType} • {pyq.examDate}
                          </p>
                        </div>
                        <div className="flex items-center text-xs text-slate-500 dark:text-slate-400">
                          <Download size={12} className="mr-1" /> {pyq.downloads}
                        </div>
                      </Link>
                    ))}
                  </div>
                    ...
                    {recentResources.videos.map(video => (
                      ...
                      >
                        ...
                        <div className="flex space-x-3 text-slate-500 dark:text-slate-400">
                          ...
                          <span className="flex items-center text-xs">
                            <Bookmark size={12} className="mr-1" /> {video.bookmarks}
                          </span>
                        </div>
                      </Link>
                    ))}
          ...
          <div className="glass-card p-6 rounded-xl">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Connect with fellow students, share notes, and discuss coursework
                </p>
              </Link>
